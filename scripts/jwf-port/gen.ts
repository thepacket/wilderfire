...
import { transpileSnippet, TranspileError, type Env } from './cwgsl.ts';
...
interface GenEntry {
  name: string; params: { name: string; def: number; int?: boolean }[]; code: string; preCode?: string; funcs: string; funcNames: string[];
...
for (let v0 of dump) {
  ...
  try {
    ...
    const params = v.params.map((p) => ({ name: p.name, def: DEFAULT_OVERRIDES[v.name]?.[p.name] ?? (p.def ?? 0), int: !!p.int }));
...
for (const e of entries) {
  ...
  item += `    params: [${e.params.map((p) => `{ name: ${JSON.stringify(p.name)}, def: ${p.def}${p.int ? ', int: true' : ''} }`).join(', ')}],\n`;
...
// JWildfire weighting fields: FastNoise (data/fastnoise.cu, from the Flam4 template) + wfieldValue() → src/gpu/wfield.wgsl.ts
{
  const fn = readFileSync(join(dataDir, 'fastnoise.cu'), 'utf8');
  const r = transpileSnippet('float q = wfieldValue(6, 1337, 1.0, 3, 0.5, 2.0, 0, 0, 2, __x, __x, __x);', [{ name: 'fastnoise', source: fn }],
    { bindings: new Map([['__x', { code: 't.x', ty: { k: 'f32' }, lvalue: true }]]) } as Env);
  const tsW = '// AUTO-GENERATED by scripts/jwf-port/gen.ts from data/fastnoise.cu — do not edit by hand.\n' +
    ...
    'export const WFIELD_WGSL = `' + esc(r.functions) + '`;\n';
  writeFileSync(join(here, '..', '..', 'src', 'gpu', 'wfield.wgsl.ts'), tsW);
}
// the deliberately-unported list (data/unportable.json) → src/core/variations.unportable.ts, so the importer can say why a variation is skipped
{
  ...
  // JWildfire variations implemented by hand in src/core/variations.ts (not through this pipeline)
  // (kernel ports, the point-set family of src/core/pointSets.ts, the CPU meshes, and the formula plot family of src/core/plots.ts)
  const HAND_PORTED = new Set(['obj_mesh_primitive_wf', 'obj_mesh_wf', 'subflame_wf', 'inversion', 'mobius3D_with_inverse', 'pre_stabilize',
    'grid3d_wf', 'dla_wf', 'scrambly', 'maurer_lines', 'tree_js', 'brownian_js', 'dragon_js', 'gosperisland_js', 'htree_js', 'koch_js', 'rsquares_js', 'hilbert_js',
    'klein_group', 'sattractor3D', 'gpattern', 'nsudoku', 'sunflower', 'sunvoroni', 'curliecue', 'szubieta', 'triantruchet', 'arctruchet', 'mandala', 'mandala2',
    'snowflake_wf', 'geometricPrimitives', 'natural_foam', 'meeple', 'point_mirror_symmetry', 'neuron3D',
    'yplot2d_wf', 'yplot3d_wf', 'parplot2d_wf', 'polarplot2d_wf', 'polarplot3d_wf', 'isosfplot3d_wf', 'knots3D', 'c_var', 'pre_c_var', 'post_c_var']);
  const missing = dump.filter((d) => !entries.some((e) => e.name === d.name) && !up.variations[d.name] && !HAND_PORTED.has(d.name)).map((d) => d.name);
