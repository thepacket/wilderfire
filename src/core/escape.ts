...
export const ESCAPE_FORMULAS = {
  mandelbrot: { label: 'Mandelbrot z^p + c', wgsl: 'cpow(z, vec2f(P.power, 0.0)) + c', power: true },
  burningship: { label: 'Burning Ship', wgsl: 'csqr(abs(z)) + c', power: false },
  tricorn: { label: 'Tricorn (Mandelbar)', wgsl: 'csqr(cconj(z)) + c', power: false },
  celtic: { label: 'Celtic', wgsl: 'vec2f(abs(z.x * z.x - z.y * z.y), 2.0 * z.x * z.y) + c', power: false },
  perpendicular: { label: 'Perpendicular Burning Ship', wgsl: 'vec2f(z.x * z.x - z.y * z.y, -2.0 * abs(z.x) * z.y) + c', power: false },
  lambda: { label: 'Lambda c·z·(1−z)', wgsl: 'cmul(c, cmul(z, vec2f(1.0, 0.0) - z))', power: false },
  phoenix: { label: 'Phoenix z² + Re(c) + Im(c)·z₋₁', wgsl: 'csqr(z) + vec2f(c.x, 0.0) + c.y * zprev', power: false, prev: true },
  magnet: { label: 'Magnet I ((z²+c−1)/(2z+c−2))²', wgsl: 'csqr(cdiv(csqr(z) + c - vec2f(1.0, 0.0), 2.0 * z + c - vec2f(2.0, 0.0)))', power: false },
  newton: { label: 'Newton z^p − 1', wgsl: 'z - cdiv(cpow(z, vec2f(P.power, 0.0)) - vec2f(1.0, 0.0), P.power * cpow(z, vec2f(P.power - 1.0, 0.0)))', power: true, convergent: true },
  nova: { label: 'Nova z − (z^p−1)/(p z^(p−1)) + c', wgsl: 'z - cdiv(cpow(z, vec2f(P.power, 0.0)) - vec2f(1.0, 0.0), P.power * cpow(z, vec2f(P.power - 1.0, 0.0))) + c', power: true, convergent: true },
  custom: { label: 'Custom formula', wgsl: '', power: true },
} as const;
...
export function defaultEscape(palette: RGB[]): EscapeLayerData {
  return {
    ...
    centerX: -0.5, centerY: 0, zoom: 1, rotation: 0, antialias: 1,
  };
}
...
/** What decides the compiled shader (everything else is a uniform): formula, custom text, mode, colourings, trap shape, AA. */
export function escapeSignature(e: EscapeLayerData): string {
  return [e.formula, e.formula === 'custom' ? e.custom : '', e.mode, e.coloring.outside, e.coloring.inside, e.coloring.trap.shape, e.coloring.trap.min ? 'min' : 'last', e.coloring.transfer, e.antialias].join('|');
}

/** The formula's WGSL (custom formulas compile here; a broken one falls back to z²+c and returns the error). */
export function escapeFormulaWgsl(e: EscapeLayerData): { wgsl: string; prev: boolean; convergent: boolean; error?: string } {
  const def = ESCAPE_FORMULAS[e.formula];
  if (e.formula !== 'custom') return { wgsl: def.wgsl, prev: 'prev' in def && !!def.prev, convergent: 'convergent' in def && !!def.convergent };
  try { return { wgsl: compileFormula(e.custom), prev: false, convergent: false }; }
  catch (err) { return { wgsl: 'csqr(z) + c', prev: false, convergent: false, error: (err as Error).message }; }
}
